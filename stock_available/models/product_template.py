...
class ProductTemplate(models.Model):
    ...
    @api.multi
    @api.depends('product_variant_ids.hidden_qty')
    def _compute_hidden_qty(self):
        ...
        for tmpl in self:
            if not tmpl.product_variant_ids:
                continue
            tmpl.hidden_qty = max(
                [v.hidden_qty for v in tmpl.product_variant_ids])
            tmpl.dock_qty = max(
                [v.dock_qty for v in tmpl.product_variant_ids])
            tmpl.return_qty = max(
                [v.return_qty for v in tmpl.product_variant_ids])
